...
// Função ajudante para gerar os links automaticamente
const createGift = (id, title, code) => {
  // Remove espaços extras do código para garantir que o link funcione
  const cleanCode = code.trim();
  return {
    id,
    name: `${title}`,
    code: cleanCode, // Mostra o código na interface se precisar
    browser: `https://totalbattle.com/en/?cmd=gift&code=${cleanCode}`,
    triumph: `https://totalbattle.com/en/triumph/?cmd=gift&code=${cleanCode}`,
    mobile: `totalbattle://gift/${cleanCode}`
  };
};

// --- 1. Links Diários (Fixos) ---
const dailyGifts = ref([
  createGift(1, "Monday: 500 Gold", "gold500"),
  createGift(2, "Tuesday: XP Boost", "xp"),
  createGift(3, "Wednesday: 1500 Tar", "tar1500"),
  createGift(4, "Thursday: 25p Speedups", "speedups25"),
  createGift(5, "Friday: 500 Gold", "gold500"),
  createGift(6, "Saturday: 4x15m Speedup", "speedups15"),
  createGift(7, "Sunday: 3h Speedups", "speedups3"),
]);

// --- 2. TB Gifts (Únicos - Extraídos da lista) ---
const oneTimeGifts = ref([
  createGift(1, "1k Gold + 2x3h Speedups", "ref3_out_fZ16"),
  createGift(2, "Portals and Teleport", "4116771"),
  createGift(3, "5k Building Materials", "3291419"),
  createGift(4, "Portal + 20 March Speedups", "2663920"),
  createGift(5, "15 Tengel Fragments", "2004847"),
  createGift(6, "Bonus Gift 1 - 3K Gold", "merry_christmas2025"),
  createGift(14, "Bonus Gift 2", "ref_4jx9ssla3kt7h99m"),
  createGift(15, "Bonus Gift 3", "ref_Bpt2zzmc1rh6g55p"),
]);

// --- 3. Event Links (Roletas e Especiais) ---
const eventGifts = ref([
  createGift(1, "Dragon Roulette Coins", "dragonroulette100"),
  createGift(2, "Lucky Roulette Coins", "luckyroulette100"),
  createGift(3, "Ramses Roulette Coins", "ramsesroulette100"),
]);
            ...
            <tr v-for="gift in section.data" :key="gift.id" class="border-b border-white/5 hover:bg-white/5 transition duration-200 group">
              ...
              <td class="p-3">
                <div class="flex justify-center gap-2">
                  ...
                  <a :href="gift.browser" target="_blank" title="Play in Browser" 
                  ...
                  <a :href="gift.triumph" target="_blank" title="Play on Triumph"
                  ...
                  <a :href="gift.mobile" title="Open in App"
